  ...
  async handleUserQuery(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (
        options.activeSince &&
        options.activeUntil &&
        options.activeSince > options.activeUntil
      ) {
        res.status(400).json({
          error: 'Invalid date range',
      ...
      if (req.query.sortBy) {
        const sortBy = req.query.sortBy as string;
        if (
          !['firstSeen', 'lastSeen', 'eventCount', 'sessionCount'].includes(
            sortBy
          )
        ) {
          res.status(400).json({
            error: 'Invalid sortBy',
            message:
              'sortBy must be one of: firstSeen, lastSeen, eventCount, sessionCount',
            statusCode: 400,
            timestamp: new Date(),
            requestId,
          });
