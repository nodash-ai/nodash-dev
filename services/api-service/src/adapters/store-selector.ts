...
export class AdapterStoreSelector implements StoreSelector {
  ...
  async healthCheck(): Promise<{
    ...
    const [eventStore, userStore, rateLimiter, deduplication] =
      await Promise.all([
  ...
  private async createEventAdapter(): Promise<EventAdapter> {
    switch (this.config.stores.events) {
      ...
      default:
        throw new Error(
          `Unsupported event storage type: ${this.config.stores.events}`
        );
    }
  }
  ...
  private async createUserAdapter(): Promise<UserAdapter> {
    switch (this.config.stores.users) {
      ...
      default:
        throw new Error(
          `Unsupported user storage type: ${this.config.stores.users}`
        );
    }
  }
  ...
  private async createRateLimitAdapter(): Promise<RateLimitAdapter> {
    switch (this.config.stores.rateLimits) {
      ...
      default:
        throw new Error(
          `Unsupported rate limit storage type: ${this.config.stores.rateLimits}`
        );
    }
  }
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error(
        'StoreSelector not initialized. Call initialize() first.'
      );
    }
  }
}
