...
describe('Query Functionality Integration Tests', () => {
  ...
  const createTestEvent = (eventName: string, properties: Record<string, any> = {}, userId?: string) => ({
    event: eventName,
    properties,
    userId,
  ...
  describe('Backend Query Service', () => {
    ...
    it('should filter events by event type', async () => {
      ...
      const pageViewResponse = await fetch(`${baseUrl}/v1/track`, {
        ...
        body: JSON.stringify(createTestEvent('page_view', { page: '/test' }, 'user1')),
      });
      ...
      const clickResponse = await fetch(`${baseUrl}/v1/track`, {
        ...
        body: JSON.stringify(createTestEvent('click', { button: 'test' }, 'user1')),
      });
      ...
      const response = await fetch(`${baseUrl}/v1/events/query?eventTypes=page_view`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
    ...
    it('should filter events by user ID', async () => {
      ...
      const user2Response = await fetch(`${baseUrl}/v1/track`, {
        ...
        body: JSON.stringify(createTestEvent('page_view', { page: '/test' }, 'user2')),
      });
    ...
    it('should handle pagination correctly', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const response = await fetch(`${baseUrl}/v1/track`, {
          ...
          body: JSON.stringify(createTestEvent('test_event', { index: i }, 'user1')),
        });
        expect(response.status).toBe(200);
      }
      ...
      // Query with limit 2
      const response = await fetch(`${baseUrl}/v1/events/query?limit=2&offset=0`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
    ...
    it('should handle invalid query parameters', async () => {
      ...
      const invalidDateResponse = await fetch(`${baseUrl}/v1/events/query?startDate=invalid-date`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
      ...
      // Test invalid limit
      const invalidLimitResponse = await fetch(`${baseUrl}/v1/events/query?limit=-1`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
      ...
      const invalidSortResponse = await fetch(`${baseUrl}/v1/events/query?sortBy=invalid_field`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
  ...
  describe('Cross-Interface Consistency', () => {
    it('should return consistent results across SDK and API', async () => {
      // Track an event
      const testEvent = createTestEvent('consistency_test', { test: true }, 'consistency_user');
      ...
      // Query via API
      const apiResponse = await fetch(`${baseUrl}/v1/events/query?eventTypes=consistency_test&limit=10`, {
        method: 'GET',
        headers: {
          'x-tenant-id': TENANT_ID,
          'x-api-key': API_KEY,
