...
export class ConfigLoader {
  static load(): Config {
    const config: Config = {
      ...
      // Database URLs
      urls: {
        ...(process.env.CLICKHOUSE_URL && { clickhouse: process.env.CLICKHOUSE_URL }),
      ...
      observability: {
        ...
        ...(process.env.OTEL_EXPORTER_OTLP_ENDPOINT && { otelEndpoint: process.env.OTEL_EXPORTER_OTLP_ENDPOINT }),
        ...(process.env.PROMETHEUS_PORT && { prometheusPort: parseInt(process.env.PROMETHEUS_PORT, 10) }),
      },
    };
  ...
  static validate(config: Config): void {
    ...
    const validStorageTypes: StorageType[] = ['flatfile', 'clickhouse', 'bigquery', 'postgres', 'dynamodb', 'memory', 'redis'];
    ...
    if (!validStorageTypes.includes(config.stores.rateLimits)) {
      errors.push(`Invalid rate limit storage type: ${config.stores.rateLimits}`);
    }
    ...
    if ((config.stores.users === 'postgres' || config.stores.events === 'postgres') && !config.urls.postgres) {
