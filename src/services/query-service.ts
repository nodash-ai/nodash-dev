...
export class QueryService {
  ...
  async queryEvents(tenantId: string, options: QueryOptions): Promise<QueryResult> {
    ...
    // Apply pagination
    const paginatedResult = this.paginateResults(events, options.limit, options.offset);
  ...
  async queryUsers(tenantId: string, options: UserQueryOptions): Promise<UserQueryResult> {
    const startTime = Date.now();
    ...
    // Apply pagination
    const paginatedResult = this.paginateResults(users, options.limit, options.offset);
  ...
  private buildEventFilter(tenantId: string, options: QueryOptions): QueryFilter {
  ...
  private buildUserFilter(tenantId: string, options: UserQueryOptions): UserQueryFilter {
  ...
  private sortEvents(events: AnalyticsEvent[], sortBy: string, sortOrder: 'asc' | 'desc'): AnalyticsEvent[] {
  ...
  private sortUsers(users: UserRecord[], sortBy: string, sortOrder: 'asc' | 'desc'): UserRecord[] {
  ...
  private paginateResults<T>(data: T[], limit?: number, offset?: number): {
    data: T[];
    hasMore: boolean;
