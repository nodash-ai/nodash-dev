...
export class TrackHandler {
  ...
  async handle(req: Request, res: Response): Promise<void> {
    try {
      ...
      const timestamp = trackRequest.timestamp ? new Date(trackRequest.timestamp) : new Date();
      ...
      const isDuplicate = await deduplicationAdapter.isDuplicate(tenantId, event.eventId);
  ...
  private async updateUserRecord(tenantId: string, userId: string, timestamp: Date): Promise<void> {
    try {
